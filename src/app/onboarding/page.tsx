...
export default function OnboardingWelcomePage() {
  ...
  return (
    <AppShell hideNav>
      <ScreenContent className="flex flex-col justify-between py-8">
        {/* Skip */}
        <div className="flex justify-end">
          {!isLast && (
            <button
              onClick={handleSkip}
              className="text-xs font-semibold text-muted hover:text-navy transition-colors px-2 py-1"
            >
              Skip
            </button>
          )}
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
          ...
          >
            <div
              className={`inline-flex items-center justify-center w-20 h-20 rounded-3xl mb-8 ${slide.color}`}
            >

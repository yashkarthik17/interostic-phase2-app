...
import { Home, Shield, Scale, Briefcase, User, ChevronLeft, Settings } from "lucide-react";
...
export function BottomNav() {
  ...
  return (
    <nav className="flex justify-around items-center px-4 pt-2 pb-5 bg-white border-t border-border shrink-0"
      style={{ boxShadow: "0 -1px 3px rgba(10,22,40,0.04)" }}>
      {navItems.map(({ href, icon: Icon, label }) => {
        const active = pathname.startsWith(href);
        return (
          <Link key={href} href={href}
            className={`flex flex-col items-center gap-1 px-3 py-1.5 text-[10px] font-bold transition-colors ${active ? "text-navy" : "text-placeholder hover:text-muted"}`}>
...
export function ScreenHeader({ title, backHref, action }: { title: string; backHref?: string; action?: React.ReactNode }) {
  return (
    <header className="flex items-center gap-3 px-6 py-2 bg-white shrink-0 z-10">
      {backHref && (
...
export function ScreenContent({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`flex-1 overflow-y-auto overflow-x-hidden px-6 pb-6 ${className || ""}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="px-6 pt-2 pb-1">
      ...
      <div className="h-1 bg-border rounded-full overflow-hidden">
        <div className="h-full bg-navy rounded-full transition-all duration-600" style={{ width: `${value}%` }} />
      </div>
    </div>
  );
}
...
export function Badge({ children, variant = "primary" }: { children: React.ReactNode; variant?: "primary" | "success" | "warning" | "danger" | "info" }) {
  const styles = {
    primary: "bg-navy-light text-navy",
...
export function Button({ children, variant = "primary", className, disabled, onClick, href, full = true }: {
  ...
  const styles = {
    primary: "bg-brand-green text-white px-7 py-4 shadow-sm hover:opacity-90",
    secondary: "bg-navy text-white px-7 py-4 hover:opacity-90",
    outline: "bg-transparent border-[1.5px] border-border-strong text-navy px-7 py-3.5 hover:border-navy",
...
export function IconCircle({ icon: Icon, color = "blue", size = 40 }: { icon: React.ElementType; color?: string; size?: number }) {
  const colors: Record<string, string> = {
    blue: "bg-navy-light text-brand-blue",
